...
![Understanding the Proxy Design Pattern | by Mithun Sasidharan | Medium](https://gitee.com/aaronlynn/picture/raw/master/img/1*DjWCgTFm-xqbhbNQVsaWQw.png)
...
1. **JDK 动态代理只能只能代理实现了接口的类或者直接代理接口，而 CGLIB 可以代理未实现任何接口的类。** 另外， CGLIB 动态代理是通过生成一个被代理类的子类来拦截被代理类的方法调用，因此<font color='red'>**不能代理声明为 final 类型的类和方法**</font>。
